...
namespace ConfigDevice
{
    ...
    /// CH20
    /// </summary>
    public class ViewLogicCH20 : BaseViewLogicControl
    {
        GridViewDigitalEdit ch20Edit = new GridViewDigitalEdit();//--CH2O编辑控件---
        GridViewComboBox cbxOperateLevel = new GridViewComboBox();//---操作运算--
        GridViewComboBox cbxCH2OLevelEdit = new GridViewComboBox();//---CH2O级别编辑控件--- 
        GridViewGridLookupEdit gridLookupDevice1;//---查找设备列表---
        DataTable dtSelectDevices1;//---选择的设备列表---
        public ViewLogicCH20(Device _device, GridView gv)
            : base(_device, gv)
        {
            ...
            //-------初始化CH2O编辑控件------
            ch20Edit.DisplayFormat.FormatString = "#0 ppb";
            ch20Edit.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            ch20Edit.Mask.EditMask = "#0 ppb";
            ch20Edit.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric;
            ch20Edit.Mask.UseMaskAsDisplayFormat = true;
            ch20Edit.MaxValue = 1000;
            ch20Edit.MinValue = -1000;
            //-------初始化级别编辑控件------
            foreach (string value in CH2OSensor.LEVEL_ID_NAME.Values)
                cbxCH2OLevelEdit.Items.Add(value);
        ...
        public override void InitViewSetting()
        {
            ...
            setGridColumnValid(dcStartValue, ch20Edit);//---开始值有效
        ...
        private void kindChanged()
        {
            ...
            if (kindName == SensorConfig.SENSOR_VALUE_KIND_VALUE)
            {
                ...
                setGridColumnValid(dcStartValue, ch20Edit);
                setGridColumnValid(dcEndValue, ch20Edit);
            ...
            {
                setGridColumnValid(dcStartValue, cbxCH2OLevelEdit);
                setGridColumnInvalid(dcEndValue);

                gvLogic.SetRowCellValue(0, dcOperate, cbxOperate.Items[0].ToString());//---第一个运算符-----
                gvLogic.SetRowCellValue(0, dcStartValue, cbxCH2OLevelEdit.Items[0].ToString());//---开始值---         
            }
            operateChanged();
        }
        ...
        private void operateChanged()
        {
            ...
            else
            {
                ...
                {
                    setGridColumnValid(dcEndValue, cbxCH2OLevelEdit);
                    gvLogic.SetRowCellValue(0, dcEndValue, cbxCH2OLevelEdit.Items[0].ToString());//---开始值---     
                }
                else
                {
                    setGridColumnValid(dcEndValue, ch20Edit);//----设置结束值有效----
                    gvLogic.SetRowCellValue(0, dcEndValue, 0);      //---结束值---
                }
        ...
        public override TriggerData GetLogicData()
        {
            ...
            {
                triggerData.Size1 = CH2OSensor.LEVEL_NAME_ID[dr[dcStartValue.FieldName].ToString()];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    triggerData.Size2 = CH2OSensor.LEVEL_NAME_ID[dr[dcEndValue.FieldName].ToString()];
            }
            else
            {
        ...
        public override void SetLogicData(TriggerData td)
        {
            ...
            {
                dcStartValue.ColumnEdit = cbxCH2OLevelEdit;
                dr[dcStartValue.FieldName] = CH2OSensor.LEVEL_ID_NAME[td.Size1];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    dr[dcEndValue.FieldName] = CH2OSensor.LEVEL_ID_NAME[td.Size2];
            }
            else
            {
