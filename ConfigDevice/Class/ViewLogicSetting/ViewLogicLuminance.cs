...
namespace ConfigDevice
{
    ...
    public class ViewLogicLuminance : BaseViewLogicControl
    {
        ...
        GridViewComboBox cbxLuminanceLevelEdit = new GridViewComboBox();//---亮度级别编辑控件--- 
            ...
            : base(_device, gv)
        {
            ...
            //-------初始化亮度编辑控件------
            luminanceEdit.DisplayFormat.FormatString = "#0 Lux";
            luminanceEdit.DisplayFormat.FormatType = DevExpress.Utils.FormatType.Numeric;
            luminanceEdit.Mask.EditMask = "#0 Lux";
            luminanceEdit.Mask.MaskType = DevExpress.XtraEditors.Mask.MaskType.Numeric;
            luminanceEdit.Mask.UseMaskAsDisplayFormat = true;
            luminanceEdit.MaxValue = 10000;
            luminanceEdit.MinValue = 0;
            //-------初始化级别编辑控件------
            foreach (string value in  LuminanceSensor.LEVEL_ID_NAME.Values)
                cbxLuminanceLevelEdit.Items.Add(value);
        ...
        private void kindChanged()
        {
            ...
            {
                setGridColumnValid(dcStartValue, cbxLuminanceLevelEdit);
                setGridColumnInvalid(dcEndValue);

                gvLogic.SetRowCellValue(0, dcOperate, cbxOperate.Items[0].ToString());//---第一个运算符-----
                gvLogic.SetRowCellValue(0, dcStartValue, cbxLuminanceLevelEdit.Items[0].ToString());//---开始值---         
            }
            operateChanged();
        }
        ...
        private void operateChanged()
        {
            ...
            else
            {
                ...
                {
                    setGridColumnValid(dcEndValue, cbxLuminanceLevelEdit);
                    gvLogic.SetRowCellValue(0, dcEndValue, cbxLuminanceLevelEdit.Items[0].ToString());//---开始值---     
                }
                else
                {
        ...
        public override void SetLogicData(TriggerData td)
        {
            ...
            {
                dcStartValue.ColumnEdit = cbxLuminanceLevelEdit;
