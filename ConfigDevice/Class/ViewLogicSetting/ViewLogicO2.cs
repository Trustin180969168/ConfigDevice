...
namespace ConfigDevice
{
    ...
    /// 氧气
    /// </summary>
    public class ViewLogicO2 : BaseViewLogicControl
    {
        GridViewPercentEdit tvocEdit = new GridViewPercentEdit();//--编辑控件---
        GridViewComboBox cbxOperateLevel = new GridViewComboBox();//---操作运算--
        GridViewComboBox cbxO2LevelEdit = new GridViewComboBox();//---O2级别编辑控件--- 
        GridViewGridLookupEdit gridLookupDevice1;//---查找设备列表---
        DataTable dtSelectDevices1;//---选择的设备列表---
        public ViewLogicO2(Device _device, GridView gv)
            : base(_device, gv)
        {
            ...
            //-------初始化级别编辑控件------
            foreach (string value in O2Sensor.LEVEL_ID_NAME.Values)
                cbxO2LevelEdit.Items.Add(value);
        ...
        private void kindChanged()
        {
            ...
            {
                setGridColumnValid(dcStartValue, cbxO2LevelEdit);
                setGridColumnInvalid(dcEndValue);

                gvLogic.SetRowCellValue(0, dcOperate, cbxOperate.Items[0].ToString());//---第一个运算符-----
                gvLogic.SetRowCellValue(0, dcStartValue, cbxO2LevelEdit.Items[0].ToString());//---开始值---         
            }
            operateChanged();
        }
        ...
        private void operateChanged()
        {
            ...
            else
            {
                ...
                {
                    setGridColumnValid(dcEndValue, cbxO2LevelEdit);
                    gvLogic.SetRowCellValue(0, dcEndValue, cbxO2LevelEdit.Items[0].ToString());//---开始值---     
                }
                else
                {
        ...
        public override TriggerData GetLogicData()
        {
            ...
            {
                triggerData.Size1 = O2Sensor.LEVEL_NAME_ID[dr[dcStartValue.FieldName].ToString()];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    triggerData.Size2 = O2Sensor.LEVEL_NAME_ID[dr[dcEndValue.FieldName].ToString()];
            }
            else
            {
        ...
        public override void SetLogicData(TriggerData td)
        {
            ...
            {
                dcStartValue.ColumnEdit = cbxO2LevelEdit;
                dr[dcStartValue.FieldName] = O2Sensor.LEVEL_ID_NAME[td.Size1];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    dr[dcEndValue.FieldName] = O2Sensor.LEVEL_ID_NAME[td.Size2];
            }
            else
            {
