...
namespace ConfigDevice
{
    ...
    public class Maglev : Device
    {
        public const string CLASS_NAME = "Maglev";
        ...
        public Short4Sensor Short4Sensor5;//----短路输入5----
        public Short4Sensor Short4Sensor6;//----短路输入6----
        public Short4Sensor Short4Sensor7;//----短路输入7----
        public Short4Sensor Short4Sensor8;//----短路输入8----
        //------短路输入参数------
        ...
        public byte ShortConfigRoad5 = 0;//短路输出5：上电初始电平 (如: SC_OUT_LOW)
        public byte ShortConfigRoad6 = 0;//短路输出6：上电初始电平 (如: SC_OUT_LOW)
        public byte ShortConfigRoad7 = 0;//短路输出7：上电初始电平 (如: SC_OUT_LOW)
        public byte ShortConfigRoad8 = 0;//短路输出8：上电初始电平 (如: SC_OUT_LOW)
        //------控制对象------
        ...
        public ShortInput Short4CtrlObj5;//---短路控制5--
        public ShortInput Short4CtrlObj6;//---短路控制6--
        public ShortInput Short4CtrlObj7;//---短路控制7--
        public ShortInput Short4CtrlObj8;//---短路控制8--

        public Maglev(UserUdpData userUdpData)
            : base(userUdpData)
        {
            initCallback();
            initControlObjs();
        ...
        private void initControlObjs()
        {
            ...
            Short4CtrlObj5 = new ShortInput(this);
            Short4CtrlObj6 = new ShortInput(this);
            Short4CtrlObj7 = new ShortInput(this);
            Short4CtrlObj8 = new ShortInput(this);
        ...
        private void getStateInfoData(UdpData data, object[] values)
        {
            ...
            this.Short4Sensor1 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_SENONAUTOOFF, dataByte1) as Short4Sensor;//短路输入1
            this.Short4Sensor2 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_SENONOFF, dataByte1) as Short4Sensor;//短路输入2
            this.Short4Sensor3 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_KEY1, dataByte1) as Short4Sensor;//短路输入3
            this.Short4Sensor4 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_KEY2, dataByte1) as Short4Sensor;//短路输入4
            this.Short4Sensor5 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_SAFETYLC, dataByte1) as Short4Sensor;//短路输入5
            this.Short4Sensor6 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_ALWAYSON, dataByte1) as Short4Sensor;//短路输入6
            this.Short4Sensor7 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_CLOSECHECK, dataByte1) as Short4Sensor;//短路输入7
            this.Short4Sensor8 = SensorCtrl.GetSensorFromByte(SensorConfig.LG_SENSOR_OPENCHECK, dataByte1) as Short4Sensor;//短路输入8
        ...
        private void getConfigData(UdpData data, object[] values)
        {
            ...
            this.ShortConfigRoad5 = value[5];
            this.ShortConfigRoad6 = value[6];
            this.ShortConfigRoad7 = value[7];
            this.ShortConfigRoad8 = value[8];
        ...
        private UdpData createWriteConfigUdp()
        {
            ...
            crcData[15] = ShortConfigRoad5;//----配置5
            crcData[16] = ShortConfigRoad6;//----配置6
            crcData[17] = ShortConfigRoad7;//----配置7
            crcData[18] = ShortConfigRoad8;//----配置8
            byte[] crc = CRC32.GetCheckValue(crcData);     //---------获取CRC校验码--------
            //---------拼接到包中------
            Buffer.BlockCopy(crcData, 0, udp.ProtocolData, 0, crcData.Length);//---校验数据---
            Buffer.BlockCopy(crc, 0, udp.ProtocolData, crcData.Length, 4);//---校验码----
        ...
        public void ReadAdditionLogic(int startNum, int endNum)
        {
            ...
            SysCtrl.AddRJ45CallBackList(DeviceConfig.CMD_PUBLIC_WRITE_END,
                EditHandleID + DeviceConfig.CMD_LOGIC_WRITE_EXACTION.ToString(), getWriteEnd2);
        ...
        private void getWriteEndData2(UdpData data, object[] values)
        {
            ...
            if (userData.SourceID == DeviceID && CommonTools.BytesEuqals(cmd, DeviceConfig.CMD_PUBLIC_WRITE_END))
            {
                UdpTools.ReplyDelRJ45SendUdp(data);//----回复确认-----
                this.CallbackUI(new CallbackParameter(ActionKind.ReadAdditionAciton, DeviceID));//---回调UI---
            }
        ...
        public void SetAdditionLogicData(byte[] value)
        {
            ...
            Short4CtrlObj5.ucScOutAct = value[21];//---高低电平
            Short4CtrlObj5.usScOutDly = ConvertTools.Bytes2ToUInt16(value[22], value[23]);//---延迟时间---
            Short4CtrlObj5.usScOutTim = ConvertTools.Bytes2ToUInt16(value[24], value[25]);//---动作时间---
            Short4CtrlObj6.ucScOutAct = value[26];//---高低电平
            Short4CtrlObj6.usScOutDly = ConvertTools.Bytes2ToUInt16(value[27], value[28]);//---延迟时间---
            Short4CtrlObj6.usScOutTim = ConvertTools.Bytes2ToUInt16(value[29], value[30]);//---动作时间---
            Short4CtrlObj7.ucScOutAct = value[31];//---高低电平
            Short4CtrlObj7.usScOutDly = ConvertTools.Bytes2ToUInt16(value[32], value[33]);//---延迟时间---
            Short4CtrlObj7.usScOutTim = ConvertTools.Bytes2ToUInt16(value[34], value[35]);//---动作时间---
            Short4CtrlObj8.ucScOutAct = value[36];//---高低电平
            Short4CtrlObj8.usScOutDly = ConvertTools.Bytes2ToUInt16(value[37], value[38]);//---延迟时间---
            Short4CtrlObj8.usScOutTim = ConvertTools.Bytes2ToUInt16(value[39], value[40]);//---动作时间---
        }
        ...
        private UdpData createWriteAdditionLogicUdp(int groupNum)
        {
            ...
            byte len = 4 + 41;//---数据长度----
            byte[] crcData = new byte[10 + 41];//----1:第几个逻辑组,5:附加数据长度
            Buffer.BlockCopy(target, 0, crcData, 0, 3);
            Buffer.BlockCopy(source, 0, crcData, 3, 3);
            crcData[6] = page;
            Buffer.BlockCopy(cmd, 0, crcData, 7, 2);
            ...
            Buffer.BlockCopy(Short4CtrlObj5.GetValue(), 0, crcData, 31, 5);//---短路输入5
            Buffer.BlockCopy(Short4CtrlObj6.GetValue(), 0, crcData, 36, 5);//---短路输入6
            Buffer.BlockCopy(Short4CtrlObj7.GetValue(), 0, crcData, 41, 5);//---短路输入7
            Buffer.BlockCopy(Short4CtrlObj8.GetValue(), 0, crcData, 46, 5);//---短路输入8
        ...
        {
            //----第一个为逻辑组号,不用设置 
            byte[] additionValue = new byte[41];
            ...
            temp = Short4CtrlObj5.GetValue();
            Buffer.BlockCopy(temp, 0, additionValue, 21, 5);
            temp = Short4CtrlObj6.GetValue();
            Buffer.BlockCopy(temp, 0, additionValue, 26, 5);
            temp = Short4CtrlObj7.GetValue();
            Buffer.BlockCopy(temp, 0, additionValue, 31, 5);
            temp = Short4CtrlObj8.GetValue();
            Buffer.BlockCopy(temp, 0, additionValue, 36, 5);
