...
原来显示分为：全部显示、只显示没关没锁，现在固定为只显示没关没锁。
        */
        ...
        // KEY_WKMD_0            0        //默认工作模式  (全部按键为通用键)◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
        // KEY_WKMD_1            1        //2键台面清洗通用键   + 3键通用键 ◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
        // KEY_WKMD_2            2        //2键上下升降柜专用键 + 3键通用键 ◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
        // KEY_WKMD_3            3        //1键新风键           + 3键通用键 ◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
        // KEY_WKMD_4            4        //2键3D抽油烟键       + 3键通用键 ◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
        // char  ucLedLumen;             //指示灯亮度   0~10               ◆当面板为[通用+专用面板]时，用作[按键功能分类]配置参数(廖超庭:2017-10-27修改)◆
