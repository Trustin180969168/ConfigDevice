...
namespace ConfigDevice
{
    ...
    /// 风向
    /// </summary>
    public class ViewLogicWindyDir : BaseViewLogicControl
    {
        ...
        public ViewLogicWindyDir(Device _device, GridView gv)
            : base(_device, gv)
        {
            ...
            //-------初始化级别编辑控件------ 
            foreach (string value in WindDirSensor.LEVEL_ID_NAME.Values)
        ...
        {
            setGridColumnValid(dcTriggerKind, cbxKind);//---触发类型有效----
            setGridColumnInvalid(dcDifferentDevice);//---默认触发设备无效----
        ...
        public override TriggerData GetLogicData()
        {
            ...
            {
                triggerData.Size1 = WindDirSensor.LEVEL_NAME_ID[dr[dcStartValue.FieldName].ToString()];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    triggerData.Size2 = WindDirSensor.LEVEL_NAME_ID[dr[dcEndValue.FieldName].ToString()];
                else
                    triggerData.Size2 = triggerData.Size1;//----强制与开始值相同------
            }
            else
            {              
                triggerData.Size1 = Convert.ToInt32(Convert.ToDouble(dr[dcStartValue.FieldName]));      
                if (dcEndValue.OptionsColumn.AllowEdit)//---有效则添加到结束值-----
                    triggerData.Size2 = Convert.ToInt32(Convert.ToDouble(dr[dcEndValue.FieldName])); 
            }
        ...
        public override void SetLogicData(TriggerData td)
        {
            ...
            if (td.TriggerKindID == SensorConfig.LG_SENSOR_LVL_FLAG)//---为级别类型---
            {
                ...
                dr[dcStartValue.FieldName] = WindDirSensor.LEVEL_ID_NAME[td.Size1];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    dr[dcEndValue.FieldName] = WindDirSensor.LEVEL_ID_NAME[td.Size2];
            }
            else
            {
