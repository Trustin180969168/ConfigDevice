...
namespace ConfigDevice
{
    ...
    public class ViewLogicWindy : BaseViewLogicControl
    {
            ...
            : base(_device, gv)
        {
            ...
            //-------初始化级别编辑控件------
            foreach (string value in WindySensor.LEVEL_ID_NAME.Values)
        ...
        public override TriggerData GetLogicData()
        {
            ...
            {
                triggerData.Size1 = WindySensor.LEVEL_NAME_ID[dr[dcStartValue.FieldName].ToString()];
        ...
        public override void SetLogicData(TriggerData td)
        {
            ...
            if (td.TriggerKindID == SensorConfig.LG_SENSOR_LVL_FLAG)//---为级别类型---
            {
                ...
                dr[dcStartValue.FieldName] = WindySensor.LEVEL_ID_NAME[td.Size1];
                if (dcEndValue.OptionsColumn.AllowEdit)
                    dr[dcEndValue.FieldName] = WindySensor.LEVEL_ID_NAME[td.Size2];
            }
            else
            {
